...
INSERT INTO customers (customer_id, first_name, last_name, customer_email) 
VALUES
...
# select product with the highest price,RETURN 1 LINE
...
HAVING 
    total_revenue = (
        ...
        FROM (
            SELECT 
                SUM(p2.price * oi2.quantity) AS daily_revenue
            FROM 
                orders o2
            JOIN 
                order_items oi2 ON oi2.order_id = o2.order_id
            JOIN 
                products p2 ON p2.product_id = oi2.product_id
            GROUP BY 
                o2.order_date
        ) AS revenue_by_day
    );
